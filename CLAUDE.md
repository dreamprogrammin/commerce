...
15. **Браузер есть — смотреть глазами, а не отговариваться его отсутствием.** Playwright лежит в `node_modules`; скрипт кладётся в корень репозитория (из `/tmp` импорт не разрешится) и запускается `node <файл>.mjs`. Открывается и локальный `pnpm dev`, и прод. Доступно всё, что нужно для проверки вёрстки: скриншоты на нужной ширине, `getComputedStyle`, ошибки консоли, коды ответов. Так уже ловились баги, невидимые ни в сборке, ни в тестах: карточки «Похожих товаров» 323px вместо 254, перенос знака ₸ на вторую строку. **Закрытых страниц нет — и не надо отговариваться логином.** Это утверждение стояло здесь и было неверным; 26 августа 2026 оно стоило живого бага на проде: правку `formatPrice` не проверили в корзине, потому что «туда без входа не попасть».

    - **Корзина и оформление логина не требуют вовсе.** Товар кладётся кликом по `button.pc-add`, дальше `/cart` и `/checkout` открываются гостю. Заказ при этом НЕ оформлять — дойти до формы и остановиться.
    - **Страницы за логином** (`/profile/**`, админка) открываются на локальной базе: взять существующего пользователя, выдать ему пароль через `PUT /auth/v1/admin/users/{id}` сервисным ключом, войти паролем, собрать куки `createServerClient` из `@supabase/ssr` с перехватом `cookies.setAll`. Полный рецепт с граблями — в [docs/HANDOFF.md](./docs/HANDOFF.md).
    - Недоступен агенту только **авторизованный путь на самом проде** — прод-ключа у сессии нет. Вот это в отчёте называть прямо.

16. **Скорость мерить только на стенде, повторяющем бой.** Локальный `node .output/server/index.mjs` отличается от Vercel четырьмя вещами, и каждая в отдельности переворачивает вывод. 24 августа 2026 на этом трижды получены неверные цифры, вплоть до «LCP упал с 7.8 с до 1.3 с» там, где на бою он вырос:
    - **не сжимает.** Vercel жмёт. Плата за размер разметки завышается втрое: сообщил владельцу +1143 мс к DCL там, где на самом деле +558.
    - **не держит кеш документа.** У главной в `routeRules` стоит `isr`, и посетитель получает готовый HTML. Локально Nitro его не держит, каждый ответ заново ходит в базу: TTFB 360 мс против 1600. Это добавляло к разнице целую секунду, которой в бою нет.
    - **говорит по HTTP/1.1.** Шесть соединений на хост вместо мультиплексирования. `fetchpriority` на таком стенде не показывает ничего — и это артефакт протокола, а не вывод.
    - **смотрит в локальную базу с пустыми бакетами.** Картинок нет, а именно они и оказались главной платой: секция, уехавшая в SSR-разметку, тянет свои картинки в критический путь. Пока их не видно, ищешь причину не там — я потратил на разметку и гидрацию несколько часов, а виноваты были картинки.

    Рабочий стенд: сборка, запущенная с прод-данными на чтение (`NUXT_PUBLIC_SUPABASE_URL` и `NUXT_PUBLIC_SUPABASE_KEY` переопределяются в рантайме, ключ публичный — он и так в разметке боевого сайта), перед ней `h2-proxy.mjs` (TLS + HTTP/2 + gzip + кеш документа). Такой стенд совпал с боем: FCP +360 против +416, LCP +392 против +384. **Мерить при DPR 3 и DPR 2** — они выбирают разные варианты картинок, и вывод разворачивается. Готовые инструменты и точный рецепт — в [docs/HANDOFF.md](./docs/HANDOFF.md).

### Отчёт

17. **Говорить, что не проверено.** Отдельным пунктом, а не умолчанием.
18. **Поправлять свои прежние утверждения**, когда они оказались неверны, — коротко и по делу.
19. **Не глушить найденное по дороге.** Если под задачей обнаружился отдельный баг — назвать его, даже если чинить не просят.
