...
useSchemaOrg([
  {
    ...
    'description':
      'Интернет-магазин детских игрушек с широким ассортиментом и быстрой доставкой по Казахстану',
    'foundingDate': '2024',
    'address': {
      '@type': 'PostalAddress',
      'addressCountry': 'KZ',
...
<template>
  <div>
    ...
    <NuxtLayout>
      <!--
        Предел на число удержанных страниц. При переполнении вытесняется та,
        на которой не были дольше всех.

        ⚠️ Поправка 23 сентября 2026: здесь стояло, что удержание включает
        сама страница (`definePageMeta({ keepalive: true })`) и что это только
        главная и /catalog. Это неверно: в Nuxt 4.4 атрибут `keepalive` у
        `<NuxtPage>` главнее флага страницы (`props.keepalive ??
        route.meta.keepalive` в `nuxt/dist/pages/runtime/page.js`), так что
        удерживаются ВСЕ страницы — проверено меткой на лендинге LEGO. Всё,
        что страница ставит «на время своей жизни», обязано сниматься в
        `onDeactivated`, а не только при уничтожении (см. `lib/shell.ts`).
