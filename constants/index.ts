...
/**
 * Ключ подсказки о высоте карточки активного заказа.
 *
 * Карточка рисуется только на клиенте (SSR-разметка главной общая для всех и
 * лежит в ISR-кеше), поэтому место под неё резервируется по прошлому визиту:
 * компонент кладёт сюда измеренную высоту, а инлайн-скрипт в <head> читает её
 * до первой отрисовки. У гостя и у того, у кого активного заказа нет,
 * подсказки нет — и резерв равен нулю.
 */
export const ACTIVE_ORDER_HEIGHT_HINT_KEY = 'uhti:active-order-height'

/** Подсказка живёт неделю: за это время заказ успевает закрыться. */
export const ACTIVE_ORDER_HEIGHT_HINT_TTL = 7 * 24 * 60 * 60 * 1000
