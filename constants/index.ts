 ...
 */
/**
 * Категории, которые остаются в индексе даже без товара.
 *
 * Правило по умолчанию — обратное: нет товара в ветке, нет страницы в индексе
 * и в карте сайта (`utils/categoryLanding.ts`). Список здесь — осознанные
 * исключения владельца.
 *
 * `katalki` — решение владельца от 16 сентября 2026. Страница собирает 329
 * показов за квартал при нуле товаров, спрос растёт (в прошлом заходе было
 * 219), и владелец собирается её наполнить. Закрывать такую — терять готовый
 * спрос; как только товар появится, строка здесь станет лишней.
 */
export const CATEGORIES_KEPT_INDEXABLE_WITHOUT_PRODUCTS = ['katalki'] as const
