# Проверка настроек Telegram уведомлений

## 🔍 Диагностика проблемы

### 1. Примените диагностическую миграцию

```bash
pnpm supabase db push
```

Эта миграция:
- ✅ Включит расширение `pg_net` (для HTTP запросов)
- ✅ Проверит наличие триггеров
- ✅ Создаст триггеры, если их нет
- ✅ Проверит настройки URL

### 2. Проверьте переменные окружения в Supabase Dashboard

Перейдите: https://supabase.com/dashboard/project/gvsdevsvzgcivpphcuai/settings/functions

Убедитесь, что установлены:

```bash
TELEGRAM_BOT_TOKEN=ваш_токен_бота
TELEGRAM_CHAT_ID=ваш_chat_id
ADMIN_SECRET=ваш_секрет (опционально)
```

### 3. Проверьте таблицу, в которую создается заказ

**Важно!** Уведомления работают только для:
- `orders` - заказы авторизованных пользователей
- `guest_checkouts` - гостевые заказы

Если вы используете другую таблицу или старую структуру - триггер не сработает.

### 4. Проверьте логи Edge Function

После создания заказа:
1. Откройте: https://supabase.com/dashboard/project/gvsdevsvzgcivpphcuai/functions/notify-order-to-telegram/invocations
2. Посмотрите, есть ли вызовы функции
3. Если вызовов нет - триггер не срабатывает
4. Если вызовы есть, но с ошибками - смотрите логи

### 5. Тестовый запрос для проверки триггера

Выполните в SQL Editor:

```sql
-- Для гостевого заказа (если используете guest_checkouts)
SELECT public.create_guest_checkout(
  '[{"product_id": "ваш_product_id", "quantity": 1}]'::jsonb,
  '{"name": "Тест", "email": "test@test.com", "phone": "+7123456789"}'::jsonb,
  'pickup'
);

-- Для пользовательского заказа (если используете orders)
-- Нужно быть авторизованным
SELECT public.create_user_order(
  '[{"product_id": "ваш_product_id", "quantity": 1}]'::jsonb,
  'pickup',
  'cash',
  NULL,
  0
);
```

### 6. Проверка вручную

Если триггер не работает, можно вызвать функцию вручную:

```sql
-- Получите ID последнего заказа
SELECT id FROM public.orders ORDER BY created_at DESC LIMIT 1;
-- или
SELECT id FROM public.guest_checkouts ORDER BY created_at DESC LIMIT 1;

-- Вызовите функцию напрямую
SELECT public.notify_order_to_telegram();
```

## 🐛 Возможные проблемы

### Проблема 1: pg_net не установлен
**Решение**: Миграция `20251219060200_check_and_fix_telegram.sql` установит его автоматически

### Проблема 2: Триггеры не созданы
**Решение**: Миграция создаст их автоматически

### Проблема 3: Переменные окружения не установлены
**Решение**: Установите в Supabase Dashboard → Settings → Edge Functions → Secrets

### Проблема 4: Старая структура таблиц
**Решение**: Убедитесь, что используете `orders` или `guest_checkouts`, а не старую структуру

### Проблема 5: RLS блокирует триггер
**Решение**: Функция использует `SECURITY DEFINER`, должна работать

## ✅ Чеклист проверки

- [ ] Применена миграция `20251219060200_check_and_fix_telegram.sql`
- [ ] Установлены переменные окружения (TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
- [ ] Триггеры существуют (проверить в SQL Editor)
- [ ] pg_net расширение включено
- [ ] Создан тестовый заказ
- [ ] Проверены логи Edge Function
- [ ] Получено уведомление в Telegram

## 📞 Если ничего не помогло

Выполните в SQL Editor и пришлите результат:

```sql
-- Проверка всех компонентов
SELECT 
  'Триггеры' as component,
  COUNT(*) as count,
  string_agg(trigger_name, ', ') as details
FROM information_schema.triggers
WHERE trigger_name IN ('trigger_notify_user_order', 'trigger_notify_guest_checkout')
UNION ALL
SELECT 
  'Функция notify_order_to_telegram' as component,
  COUNT(*) as count,
  NULL as details
FROM information_schema.routines
WHERE routine_schema = 'public' 
  AND routine_name = 'notify_order_to_telegram'
UNION ALL
SELECT 
  'Расширение pg_net' as component,
  COUNT(*) as count,
  NULL as details
FROM pg_extension
WHERE extname = 'pg_net'
UNION ALL
SELECT 
  'Настройки URL' as component,
  COUNT(*) as count,
  value->>'url' as details
FROM public.settings 
WHERE key = 'telegram_function_url';
```
