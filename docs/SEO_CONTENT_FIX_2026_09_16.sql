...
--  SQL-функции `generate_category_questions`, `generate_brand_questions`,
--  `generate_product_line_questions` и шаблоны в `composables/useSeoTemplates.ts`.
--  Пока в них лежат те же формулировки, любая новая генерация вернёт ложные
--  обещания обратно. Тела функций агенту не видны (нужен сервисный доступ);
--  чтобы их починить, выполните и пришлите вывод:
--
--      SELECT p.proname, pg_get_functiondef(p.oid)
--        FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
--       WHERE n.nspname = 'public' AND p.proname LIKE 'generate%questions%';
--
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ── 3. «СКИДКИ ДО 50%» ─────────────────────────────────────────────────────
--
--  Размер скидки у каждого товара свой и виден на карточке; «до 50%» никто не
--  считал. Такую же строку я убрал из описания витрины «Акции».
--  Предложение удаляется целиком вместе с восклицательным знаком.
--
--  Ожидаемо: 3 строки.

--  Строк всего три, и в каждой фраза своя, поэтому вырезаем их дословно, а не
--  выражением: общее правило «убрать предложение со словами до 50%» спотыкалось
--  на «Акция! Скидки до 50%</strong>…» — восклицательный знак стоит ВНУТРИ
--  предложения, и в тексте остался бы незакрытый <strong>.
--
--  Порядок важен: этот раздел идёт ДО раздела 4, иначе 🔥 уже удалён и
--  соседний пробел ломает совпадение. Впрочем, 🔥 в образцах ниже нет — он
--  уберётся следующим разделом.

UPDATE public.category_questions
   SET answer_text = trim(regexp_replace(
         replace(
           replace(
             replace(answer_text,
               '<strong>Скидки до 50%</strong> на популярные модели! ', ''),
             '<strong>Акция! Скидки до 50%</strong> на избранные товары. ', ''),
           'Сейчас действуют <strong>скидки до 50%</strong>! ', ''),
         '\s{2,}', ' ', 'g'))
 WHERE position('до 50%' in answer_text) > 0;
