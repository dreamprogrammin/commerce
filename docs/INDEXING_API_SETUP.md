# 🚀 Google Indexing API - Автоматический запрос индексации

## Что это?

**Google Indexing API** - официальный API от Google, который позволяет программно запрашивать индексацию URL без ручного ввода в Google Search Console.

**Преимущества:**

- ✅ Автоматический запрос индексации для всех URL
- ✅ До 200 URL в день (vs 10 вручную)
- ✅ Можно интегрировать в CI/CD
- ✅ Быстрее чем ручной запрос

**Лимиты:**

- 📊 200 URL в день
- 🔄 100 запросов в минуту
- 📈 Можно запросить увеличение квоты

---

## 📋 Шаг 1: Создайте Google Service Account

### 1.1. Откройте Google Cloud Console

Перейдите: https://console.cloud.google.com/

### 1.2. Создайте новый проект (или выберите существующий)

1. Вверху нажмите на название проекта
2. **"Новый проект"**
3. Название: `uhti-indexing` (или любое другое)
4. Нажмите **"Создать"**

### 1.3. Включите Indexing API

1. Левое меню → **"API и сервисы"** → **"Библиотека"**
2. Найдите: `Indexing API`
3. Нажмите на результат
4. Нажмите **"Включить"**

### 1.4. Создайте Service Account

1. Левое меню → **"API и сервисы"** → **"Учетные данные"**
2. Вверху нажмите **"+ Создать учетные данные"**
3. Выберите **"Сервисный аккаунт"**
4. Заполните:
   - **Имя:** `uhti-indexing-bot`
   - **Описание:** `Bot for automatic indexing requests`
5. Нажмите **"Создать и продолжить"**
6. **Роль:** выберите `Owner` (Владелец) или `Editor` (Редактор)
7. Нажмите **"Продолжить"** → **"Готово"**

### 1.5. Создайте JSON ключ

1. В списке сервисных аккаунтов найдите созданный `uhti-indexing-bot`
2. Нажмите на него
3. Вкладка **"Ключи"**
4. **"Добавить ключ"** → **"Создать новый ключ"**
5. Тип: **JSON**
6. Нажмите **"Создать"**
7. Файл `service-account.json` скачается автоматически

**⚠️ ВАЖНО:** Храните этот файл в секрете! Не коммитьте в git!

### 1.6. Скопируйте email сервисного аккаунта

Он выглядит так:

```
uhti-indexing-bot@uhti-indexing.iam.gserviceaccount.com
```

---

## 📋 Шаг 2: Добавьте Service Account в Google Search Console

### 2.1. Откройте Google Search Console

Перейдите: https://search.google.com/search-console

### 2.2. Добавьте владельца

1. Выберите ресурс `uhti.kz`
2. Левое меню → **"Настройки"** (внизу)
3. Раздел **"Пользователи и разрешения"**
4. Нажмите **"Добавить пользователя"**
5. Email: вставьте email сервисного аккаунта
   ```
   uhti-indexing-bot@uhti-indexing.iam.gserviceaccount.com
   ```
6. Права: **"Владелец"** (Owner)
7. Нажмите **"Добавить"**

---

## 📋 Шаг 3: Установите зависимости

```bash
cd /path/to/project
npm install googleapis
```

Или если используете pnpm:

```bash
pnpm add googleapis
```

---

## 📋 Шаг 4: Положите ключ в проект

1. Скачанный файл `service-account.json` положите в **корень проекта**
2. **ВАЖНО:** Добавьте в `.gitignore`:

```bash
# .gitignore
service-account.json
*.json.key
```

Проверьте:

```bash
cat .gitignore | grep service-account
```

---

## 📋 Шаг 5: Запустите скрипт

```bash
node scripts/request-indexing.js
```

**Что произойдет:**

```
🚀 Запуск автоматической индексации...

🔄 Получаю список URL из /api/sitemap-routes...
✅ Получено 23 URL из sitemap

📊 Всего URL к обработке: 23
⏱️  Примерное время: 1 минут

✅ Google Indexing API инициализирован
📧 Service Account: uhti-indexing-bot@uhti-indexing.iam.gserviceaccount.com

[1/23] Обработка: https://uhti.kz/
  ✅ Успешно отправлено
[2/23] Обработка: https://uhti.kz/catalog
  ✅ Успешно отправлено
[3/23] Обработка: https://uhti.kz/catalog/girls
  ✅ Успешно отправлено
...

📊 РЕЗУЛЬТАТЫ:
✅ Успешно: 23
❌ Ошибок: 0

✅ Готово! Индексация запрошена.
⏳ Google обработает запросы в течение 1-3 дней.
```

**Преимущества:**

- ✅ Использует ваш существующий API endpoint `/api/sitemap-routes`
- ✅ Никакого дублирования кода или логики
- ✅ Автоматически индексирует все страницы из sitemap
- ✅ При добавлении нового товара - просто запустите скрипт снова
- ✅ Соблюдает лимит 200 URL в день автоматически

---

## 🔄 Автоматизация (опционально)

### Вариант 1: Запуск после деплоя (GitHub Actions)

Создайте файл `.github/workflows/request-indexing.yml`:

```yaml
name: Request Google Indexing

on:
  push:
    branches:
      - main
  workflow_dispatch: # Ручной запуск

jobs:
  request-indexing:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v3

      - name: Setup Node.js
        uses: actions/setup-node@v3
        with:
          node-version: '18'

      - name: Install dependencies
        run: npm install googleapis

      - name: Create service account file
        run: echo '${{ secrets.GOOGLE_SERVICE_ACCOUNT }}' > service-account.json

      - name: Request indexing
        run: node scripts/request-indexing.js
```

**Настройка секрета:**

1. GitHub → Settings → Secrets → New repository secret
2. Name: `GOOGLE_SERVICE_ACCOUNT`
3. Value: содержимое `service-account.json` (весь JSON)

---

### Вариант 2: Запуск по расписанию (cron)

На сервере:

```bash
# Открыть crontab
crontab -e

# Добавить строку (каждый день в 3:00 утра)
0 3 * * * cd /path/to/project && node scripts/request-indexing.js >> /var/log/indexing.log 2>&1
```

---

### Вариант 3: Вызов из Nuxt (при добавлении товара)

Создайте `/server/api/request-indexing.post.ts`:

```typescript
import fs from 'node:fs'
import path from 'node:path'
import { google } from 'googleapis'

export default defineEventHandler(async (event) => {
  // Проверка авторизации (только админ)
  const user = await serverSupabaseUser(event)
  if (!user) {
    throw createError({
      statusCode: 401,
      message: 'Unauthorized',
    })
  }

  // Получаем URL из тела запроса
  const { url } = await readBody(event)

  if (!url || !url.startsWith('https://uhti.kz/')) {
    throw createError({
      statusCode: 400,
      message: 'Invalid URL',
    })
  }

  try {
    // Инициализация API
    const keyFilePath = path.join(process.cwd(), 'service-account.json')

    if (!fs.existsSync(keyFilePath)) {
      throw new Error('Service account file not found')
    }

    const auth = new google.auth.GoogleAuth({
      keyFile: keyFilePath,
      scopes: ['https://www.googleapis.com/auth/indexing'],
    })

    const authClient = await auth.getClient()
    const indexing = google.indexing({
      version: 'v3',
      auth: authClient,
    })

    // Запрос индексации
    const response = await indexing.urlNotifications.publish({
      requestBody: {
        url,
        type: 'URL_UPDATED',
      },
    })

    return {
      success: true,
      url,
      data: response.data,
    }
  }
  catch (error: any) {
    throw createError({
      statusCode: 500,
      message: error.message,
    })
  }
})
```

**Использование в компоненте:**

```typescript
// При создании товара в админке
async function createProduct(productData) {
  // Создать товар
  const product = await productsStore.createProduct(productData)

  // Запросить индексацию
  await $fetch('/api/request-indexing', {
    method: 'POST',
    body: {
      url: `https://uhti.kz/catalog/products/${product.slug}`,
    },
  })
}
```

---

## ℹ️ Как работает скрипт

Скрипт автоматически получает все URL из вашего существующего API endpoint:

**Откуда берутся URL:**

- Вызывается ваш endpoint: `https://uhti.kz/api/sitemap-routes`
- Этот endpoint уже реализован в `/server/api/sitemap-routes.ts`
- Он возвращает список всех страниц: категории, товары, бренды, статические страницы
- Скрипт отправляет каждый URL в Google Indexing API

**Что индексируется:**

- Всё, что есть в вашем sitemap.xml
- Категории, товары, бренды, статические страницы
- Только активные записи из базы данных

**Лимит Google:**

- Если найдено больше 200 URL → будут обработаны первые 200
- Остальные можно запросить на следующий день

---

## 🐛 Troubleshooting

### Ошибка: "Ошибка получения URL из sitemap API"

**Причина:** Не удалось получить данные из `/api/sitemap-routes`

**Решение:**

1. Проверьте что сайт доступен: `https://uhti.kz`
2. Проверьте что API работает: `https://uhti.kz/api/sitemap-routes`
   ```bash
   curl https://uhti.kz/api/sitemap-routes
   ```
3. Проверьте интернет соединение
4. Убедитесь что сайт задеплоен на production

---

### Ошибка: "Не найдено URL для индексации"

**Причина:** API вернул пустой массив

**Решение:**

1. Проверьте что в базе данных есть данные (категории, товары, бренды)
2. Проверьте что endpoint работает:
   ```bash
   curl https://uhti.kz/api/sitemap-routes | jq length
   ```
3. Убедитесь что у записей заполнено поле `slug`

---

### Ошибка: "Permission denied"

**Причина:** Service Account не добавлен в Google Search Console

**Решение:**

1. Проверьте email сервисного аккаунта
2. Добавьте его в GSC как Владельца

---

### Ошибка: "API not enabled"

**Причина:** Indexing API не включен в Google Cloud

**Решение:**

1. Google Cloud Console → API и сервисы → Библиотека
2. Найдите "Indexing API"
3. Нажмите "Включить"

---

### Ошибка: "Quota exceeded"

**Причина:** Превышен дневной лимит 200 URL

**Решение:**

1. Подождите до следующего дня
2. Или запросите увеличение квоты в Google Cloud Console

---

### Ошибка: "service-account.json not found"

**Причина:** Файл не найден или не в корне проекта

**Решение:**

1. Проверьте что файл лежит в корне проекта
2. Проверьте имя файла (должно быть точно `service-account.json`)

---

## 📊 Мониторинг

### Проверка статуса в GSC

1. Google Search Console → Индексирование страниц
2. Через 1-3 дня должно появиться больше проиндексированных страниц

### Проверка в Google поиске

```
site:uhti.kz
```

**Прогресс:**

- День 1: 3-5 страниц
- День 3: 10-15 страниц
- День 5: 20-25 страниц

---

## 🎯 Итоговый чеклист

- [ ] Создан Google Cloud проект
- [ ] Включен Indexing API
- [ ] Создан Service Account
- [ ] Скачан JSON ключ
- [ ] Service Account добавлен в GSC как Владелец
- [ ] Файл `service-account.json` положен в корень проекта
- [ ] Добавлен `service-account.json` в `.gitignore`
- [ ] Установлены зависимости (`npm install googleapis`)
- [ ] Запущен скрипт (`node scripts/request-indexing.js`)
- [ ] Проверен результат в консоли (все ✅)
- [ ] Через 3 дня: проверить прогресс в GSC

---

## 📚 Полезные ссылки

- [Google Indexing API Docs](https://developers.google.com/search/apis/indexing-api/v3/quickstart)
- [Google Cloud Console](https://console.cloud.google.com/)
- [Google Search Console](https://search.google.com/search-console)
- [googleapis npm package](https://www.npmjs.com/package/googleapis)

---

**Дата:** 2026-01-09
**Автор:** Claude Code
**Статус:** Готов к использованию
