...
> ## ⚠️ Поправка от 20 августа 2026: совет ниже про `'/**/'` — НЕВЕРНЫЙ
>
> В разделе «Что исправлено» этот документ рекомендует добавить `'/**/'` в
> `disallow` с комментарием «wildcard паттерны». Так и было сделано, и правило
> прожило в `nuxt.config.ts` до 20 августа 2026, когда его убрали.
>
> **Откуда взялась ошибка.** Search Console перечислял
> `https://uhti.kz/**/` среди «заблокированных в robots.txt» — это был сам
> шаблон из robots.txt, а не адрес, который нужно закрыть. Его приняли за
> адрес и «закрыли» повторно.
>
> **Что правило делало.** Google выбирает правило по самому длинному
> совпадению, поэтому `/**/` перекрывал любой путь из двух и более сегментов,
> кроме спасённых `Allow: /catalog/**` и `Allow: /brand/**`. Под запретом
> оказались все генерируемые OG-картинки (`/__og-image__/**`, на них ссылается
> `og:image` каталога, категорий и брендов) и весь маршрут `/promo/**`.
>
> Подробности — в `docs/SEO_AUDIT_2026_08_20.md`, пункт 8.
