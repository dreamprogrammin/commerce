# Управление кешем в приложении

## Проблема, которую мы решили

При редактировании товаров в админ-панели изменения (особенно порядок изображений) не отображались сразу из-за кеширования TanStack Query. Пользователь редактировал главное изображение, сохранял, но изменения не были видны.

## Решение

### 1. Composable `useCacheManager`

Создан универсальный composable для управления кешем:
...
// Жесткая перезагрузка (очистка + reload)
hardReset()
```

### 2. Автоматическая очистка при сохранении

Страница редактирования товара (`pages/admin/products/[id].vue`):
...
async function handleUpdate(payload) {
  ...
  // ✅ Пользователь остается на странице и видит обновленные данные
}
```

**Важно**: Убрали автоматический редирект на `/admin/products` после сохранения - теперь пользователь видит обновленные данные сразу.
...
function setPrimaryExistingImage(index: number) {
  ...
  existingImages.value.unshift(image) // Перемещаем в начало
}
```
