...
Добавлен 301 редирект с trailing slash на версию без него:
```typescript
nitro: {
  routeRules: {
    "/**": {
...
**Файл:** `middleware/trailing-slash.global.ts` (новый)

Дополнительная проверка на клиенте для редиректа с trailing slash.
