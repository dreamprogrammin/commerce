...
export interface PhotoViewerImage {
  /**
   * Полноразмерный файл. Просмотр во весь экран сознательно берёт САМЫЙ
   * крупный вариант и не выбирает по плотности экрана: кадр здесь и так на всё
   * окно, да ещё увеличивается щипком до 4×, поэтому «подходящий по ширине»
   * вариант всегда оказывается мелковат. Раньше тут был srcset, и на телефоне
   * браузер честно выбирал `_md` (600px) — фото в лайтбоксе выходило мягче,
...
/*
 * Догрузки покрупнее при увеличении больше нет — и не нужно.
 *
 * Раньше кадр приходил со srcset, браузер на телефоне выбирал `_md`, а при
 * щипке приходилось незаметно подменять файл на крупный: менять `sizes` прямо
 * на жесте нельзя, браузер бросает текущую картинку и на её месте посреди
 * щипка пустота. Теперь крупный вариант стоит сразу, подменять нечего.
 */
