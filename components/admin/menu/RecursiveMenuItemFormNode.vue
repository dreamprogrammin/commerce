  ...
  >
    ...
    <div>
      ...
      <Input
        ...
        class="w-full"
      />
       <div v-if="item._imagePreviewUrl || item.image_url" class="mt-2 border p-2 rounded-md inline-block relative">
          <img
            :src="item._imagePreviewUrl || getPublicUrl(BUCKET_NAME, item.image_url!) || undefined"
            alt="Превью"
          ...
          <Button
            ...
            type="button" aria-label="Удалить изображение"
          >
    ...
    <div class="pt-3 mt-3 border-t" v-if="item.children && item.children.length > 0">
      <div class="flex items-center justify-between">
        ...
        <Button @click="isChildrenVisible = !isChildrenVisible" size="sm" variant="ghost">
          ...
          <Icon name="lucide:chevron-down" :class="{ 'rotate-180': !isChildrenVisible }" class="ml-1 h-4 w-4 transition-transform" />
        </Button>
      </div>
    ...
    <Button @click="emit('add-child', item)" size="sm" variant="outline" class="mt-2 border-dashed w-full">
      <Icon name="lucide:plus" class="mr-1 h-4 w-4" /> Добавить подпункт (Уровень {{ level + 3 }})
    </Button>
  </div>
</template>