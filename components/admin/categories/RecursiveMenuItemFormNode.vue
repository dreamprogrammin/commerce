...
<template>
  ...
  -->
  <div :class="{ 'opacity-50 border-l-2 border-destructive pl-4 transition-opacity': item._isDeleted }">
    <div
    ...
    >
      ...
      <p class="font-medium text-sm text-foreground pr-16">Редактирование категории (Уровень {{ level + 2 }})</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
        ...
        <div>
          ...
          <Input :id="`slug-${item._tempId || item.id}`" v-model="item.slug" required :disabled="item._isDeleted" />
        </div>
      </div>
      <div>
        <Label :for="`href-${item._tempId || item.id}`">Ссылка (URL) *</Label>
        <Input :id="`href-${item._tempId || item.id}`" v-model="item.href" required :disabled="item._isDeleted" />
      </div>
      <div>
      ...
      <div>
        ...
        <Input
          ...
          :disabled="item._isDeleted"
        />
        <div v-if="item.image_url" class="mt-2 border p-2 rounded-md inline-block relative bg-background">
          <img
      ...
      <div class="pt-3 mt-3 border-t" v-if="item.children && item.children.length > 0">
        <div class="flex items-center justify-between">
          <h4 class="font-semibold text-sm text-muted-foreground">
            Подкатегории для "{{ item.name }}" ({{ item.children.filter(c => !c._isDeleted).length }} шт.)
          </h4>
          <Button @click="isChildrenVisible = !isChildrenVisible" size="sm" variant="ghost">
