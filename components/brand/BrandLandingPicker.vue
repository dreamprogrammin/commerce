...
<template>
  <section class="blp">
    <div class="blp__inner">
      ...
      <!--
        `position` НЕ передаём намеренно. Проп означает «карточка над сгибом,
        грузи первый кадр сразу», и в сетке каталога это верно. Здесь сетка
        лежит ниже шапки, полосы условий и мозаики серий — над сгибом её не
        видно ни на одном экране, а четыре картинки уходили в сеть с
        `fetchpriority="high"` и отбирали канал у той единственной, по которой
        считается LCP. На `/brand/lego` таких приоритетных картинок набралось
        одиннадцать (замер по бою 15 сентября 2026).
      -->
      <div class="blp__grid">
        <ProductCard
