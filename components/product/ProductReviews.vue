...
const imageFiles = ref<
  { file: File; preview: string; blurPlaceholder?: string }[]
>([]);
...
<template>
  ...
  >
    ...
    <div
      v-if="reviews?.length"
      class="flex flex-col sm:flex-row gap-6 mb-6 pb-6 border-b"
    >
      <!-- Средний рейтинг -->
      <div class="text-center sm:text-left">
        ...
        <div class="text-sm text-muted-foreground mt-1">
          ...
          {{
            reviewCount === 1
              ? "отзыв"
              : reviewCount && reviewCount >= 2 && reviewCount <= 4
                ? "отзыва"
                : "отзывов"
          }}
        </div>
      ...
      <div class="flex-1 space-y-1.5">
        ...
        >
          <span class="w-4 text-right text-muted-foreground">{{
            item.stars
          }}</span>
          <Icon
    ...
    <div v-if="isFormOpen" class="mb-6 p-4 bg-muted/50 rounded-lg border">
      <h3 class="font-medium mb-3">Ваш отзыв</h3>
      <div class="mb-3">
      ...
      <div class="flex gap-2">
        ...
        <Button variant="outline" @click="isFormOpen = false"> Отмена </Button>
      </div>
    </div>
    ...
    <div v-if="isLoading" class="py-8 text-center">
      <div
        class="w-8 h-8 mx-auto border-2 border-primary border-t-transparent rounded-full animate-spin"
      />
    </div>
    ...
    <div v-else class="py-8 text-center">
      <Icon
        ...
        class="w-12 h-12 mx-auto text-muted-foreground/40 mb-3"
      />
      <p class="text-muted-foreground">Пока нет отзывов</p>
