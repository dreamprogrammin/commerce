import type { CategoryInsert, CategoryRow, CategoryUpdate, Database, EditableCategory } from "@/types";
...
export const useAdminCategoriesStore = defineStore('adminCategoriesStore', () => {
  ...
  async function fetchAllCategories(force = false) {
    if (allCategories.value.length > 0 && !force) return;
    ...
    try {
      ...
      toast.error('Ошибка загрузки категорий', { description: (e as Error).message });
    } finally {
  ...
  async function saveChanges(itemsToSave: EditableCategory[]) {
    ...
    function collectChanges(items: EditableCategory[], parentId: string | null = null) {
      items.forEach((item, index) => {
        const currentItem = { ...item, parent_id: parentId, display_order: index };

        if (item._isDeleted && item.id) {
          itemsToDelete.push(item.id);
        } else if (item._isNew) {
          const { id, created_at, updated_at, children, _tempId, _isNew, _isDeleted, ...insertData } = currentItem;
          itemsToInsert.push(insertData);
        } else if (item.id) {
          const { id, created_at, updated_at, children, _tempId, _isNew, _isDeleted, ...updateData } = currentItem;
    ...
    try {
      ...
      if (itemsToInsert.length > 0) {
        const { error } = await supabase.from('categories').insert(itemsToInsert);
        if (error) throw error;
      }
      ...
      toast.error('Ошибка сохранения категорий', { description: (e as Error).message });
      return false;
    }
  }

  return { allCategories, isLoading, fetchAllCategories, buildCategoryTree, saveChanges };
});