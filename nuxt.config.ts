...
export default defineNuxtConfig({
  ...
  nitro: {
    routeRules: {
      ...
      '/': { swr: 600 },
      /*
       * Кеш страниц категорий. Возвращён после разбора 17 августа.
       *
       * Даёт много: первый байт категории падает с 2.7-3.8 с до 0.49 с,
       * LCP — с 4976 до 2492 мс, то есть под порог Google в 2500.
       *
       * История, чтобы её не повторяли. Правило уже включалось и было
       * откачено: `/catalog/boys?brand=mattel` отдавал полную категорию
       * вместо товаров бренда, и я объяснил это подменой query-строки в
       * маршруте vercel-пресета (`dest` вида `…-isr?__isr_route=$…`).
       * Объяснение оказалось неверным: тот же симптом воспроизводился на
       * превью БЕЗ ISR. Ломалось другое — id бренда резолвился из
       * `availableBrands`, а тот наполняется уже после выборки товаров.
       * Починено отдельно, id теперь берётся из адреса до запроса товаров.
       *
       * `isr`, а не `swr`, намеренно. В сборке видна разница: `swr: N` даёт
       * пресету `{"expiration": false}` — кеш живёт до следующей выкатки и
       * сам не обновляется. Так сейчас устроены `/` и карточки товара: их
       * содержимое заморожено между деплоями. `isr: N` даёт
       * `{"expiration": 1800}`, то есть страница сама переспрашивает данные
       * раз в полчаса. Для списка товаров это важнее: туда приходят новинки
       * и уходит распроданное.
       *
       * Что проверить при следующем касании — именно то, на чём я
       * ошибся: открыть бренд-лендинг из sitemap и сравнить число карточек
       * с числом товаров бренда в базе. Заголовок и H1 бренд показывают
       * правильно даже когда выдача нефильтрованная, глазами не видно.
       */
      '/catalog/**': { isr: 1800 },
      '/catalog': { swr: 1800 },
      '/catalog/products/**': {
