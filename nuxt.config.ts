...
export default defineNuxtConfig({
  ...
  nitro: {
    routeRules: {
       ...
       * ссылке с фильтром, либо фильтры и первый байт 2.7-3.8 с. Оставлен
       * ISR: внутри сайта фильтры работают (переход клиентский, сервера не
       * касается), а сами фильтрованные адреса закрыты `noindex`. Ломается
       * только прямой заход по присланной ссылке с параметром.
       *
        ...
        output: {
          ...
          manualChunks(id) {
            if (id.includes('node_modules')) {
               ...
               *  • отпустить сюда же `reka-ui` — жадный чанк не изменился
               *    вовсе, все 134.2 КБ остались. Пробовалось ДВАЖДЫ: и до,
               *    и после удаления мёртвых глобальных компонентов, которые
               *    тянули Slider с Checkbox. Оба раза ноль. Значит примитивы
               *    достижимы из жадного графа обычными импортами, и лечится
               *    это не настройкой сборки. Проверены и отброшены ещё две
               *    догадки: глобальная регистрация из `components/global/`
               *    (после чистки чанк сдвинулся на 3 КБ) и регистрация
               *    компонентов shadcn как глобальных (в модуле
               *    `shadcn-nuxt` признака `global` нет вовсе);
               *  • отпустить `vue-sonner` и сделать Toaster асинхронным —
               *    тоже ноль: `toast` импортируют ещё 75 файлов, и
               *    библиотека остаётся жадной независимо от Toaster.
               */
