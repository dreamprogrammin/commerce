...
export default defineNuxtConfig({
  ...
  nitro: {
    routeRules: {
      ...
      '/': { swr: 600 },
      /*
       * ISR для категорий НЕ ВКЛЮЧАТЬ. Проверено дважды, второй раз — чисто.
       *
       * Соблазн большой: правило `'/catalog/**': { isr: 1800 }` роняет первый
       * байт категории с 2.7-3.8 с до 0.49 с, а LCP — с 4976 до 2492 мс, то
       * есть под порог Google. Ради этого его уже включали дважды.
       *
       * И дважды снимали, потому что оно ломает бренд-лендинги. Vercel-пресет
       * генерирует маршрут
       *
       *   src:  (?<__isr_route>/catalog/(?:.*))
       *   dest: /catalog/[...]-isr?__isr_route=$__isr_route
       *
       * и query-строка из `dest` заменяет исходную — функция не получает
       * `brand=`. Замер на одном и том же коде, разница только в этой строке:
       *
       *   без ISR   /catalog/boys?brand=mattel   1 карточка,  h1 «Мальчикам Mattel в Алматы»
       *   с ISR                                 12 карточек, h1 «Мальчикам»
       *
       * Первый раз вывод был сделан поспешно: тогда бренд-лендинги были
       * сломаны и сами по себе (id бренда резолвился после выборки товаров,
       * починено в 43a50e1), так что доказательства не было. Теперь есть.
       *
       * Таких адресов 14, все в sitemap, и это ЕДИНСТВЕННЫЕ страницы с
       * параметром, которые задуманы для индекса. Менять их выдачу на общую
       * категорию ради скорости нельзя.
       *
       * Что открывает дорогу обратно: перевести бренд-лендинги с `?brand=` на
       * путь вида `/catalog/boys/brand/mattel`. Тогда ISR можно включить,
       * ничего не ломая, — но это меняет адреса в sitemap и canonical,
       * решение владельца.
       *
       * Пока ускорять надо не маршрут, а рендер: круг до Supabase стоит
       * 400-650 мс, а страница делает их волнами. Список категорий уже
       * закеширован (server/api/categories.get.ts).
       */
      '/catalog': { swr: 1800 },
      '/catalog/products/**': {
        swr: 3600,
