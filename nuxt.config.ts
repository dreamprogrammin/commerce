...
export default defineNuxtConfig({
  ...
  nitro: {
    routeRules: {
      ...
      '/': { swr: 600 },
      /*
       * ISR для категорий пробовался и ОТКАЧЕН. Оставлено как предупреждение.
       *
       * Правило `'/catalog/**': { isr: 1800 }` действительно снимало главную
       * проблему LCP: первый байт категории падал с 2.7-3.8 с до 0.49 с,
       * а сам LCP — с 4976 до 2492 мс, то есть под порог Google.
       *
       * И всё же оно ломает фильтры. Vercel-пресет генерирует маршрут
       *
       *   src:  (?<__isr_route>/catalog/(?:.*))
       *   dest: /catalog/[...]-isr?__isr_route=$__isr_route
       *
       * где query-строка в `dest` ЗАМЕНЯЕТ исходную. Функция не получает ни
       * `brand=`, ни фильтров: `/catalog/boys?brand=mattel` отдавал обычную
       * страницу категории с 12 товарами вместо бренд-лендинга с одним.
       * Кеш при этом ключует по полному адресу, так что у каждого фильтра
       * свой ключ — и в каждом лежит одна и та же нефильтрованная страница.
       * Проверено: локальная сборка того же кода фильтрует правильно, ломает
       * именно слой ISR.
       *
       * Почему `/` и `/catalog/products/**` под ISR живут нормально: там
       * query-параметры ни на что не влияют.
       *
       * Что можно сделать вместо: кешировать не страницу, а тяжёлые данные
       * на сервере. Список из 64 категорий — 174 КБ и около секунды на
       * каждый рендер, и он одинаков для всех страниц каталога.
       */
      '/catalog': { swr: 1800 },
      '/catalog/products/**': {
        swr: 3600,
