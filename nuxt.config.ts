...
export default defineNuxtConfig({
  ...
  fonts: {
    defaults: {
      ...
      styles: ['normal'],
      /*
       * `latin-ext` подключён ради ОДНОГО символа — знака тенге ₸ (U+20B8).
       *
       * В подмножествах `latin` и `cyrillic` его нет, а стоит он на каждой
       * цене: на главной 43 раза. Пока подмножества с ним нет, браузер обязан
       * взять единственные грани, которые его покрывают, — несокращённые
       * легаси-`.woff` по 52 КБ. Замер 25 августа 2026: так качалось 106 КБ
       * из 188 КБ всех шрифтов страницы, при том что сам `latin-ext` весит
       * 34 КБ.
       *
       * Вторая половина той же истории — стрелка `&rarr;` (U+2192) в
       * ProductsCarousel: её в подмножествах тоже нет (у Google в `latin`
       * есть ↑ и ↓, а → нет). Заменена на иконку. Нужны обе правки: любой
       * непокрытый символ в видимом тексте тянет полную грань целиком.
       */
    ...
    display: 'swap',
    /*
     * Предзагрузки шрифтов НЕТ, и это проверено, а не забыто.
     *
     * Здесь стоял `preload: true` — ключ, которого модуль не читает вовсе:
     * в типах опций его нет, а `shouldPreload` смотрит только на
     * `defaults.preload` и `families[].preload`. В разметке боевого сайта и
     * превью 25 августа 2026 — ноль тегов `<link rel="preload" as="font">`.
     *
     * Переносить ключ в `defaults` нельзя: там он включит предзагрузку всех
     * граней, у одного Nunito их 18 (шесть весов × подмножества). На медленном
     * канале это отберёт канал у картинок и скриптов ради CLS 0.0058 при
     * пороге 0.1.
     *
     * Метрические запасные грани (`Nunito Fallback: Arial` и прочие, с
     * `size-adjust`) модуль создаёт и подставляет в `--font-sans` сам, без
     * настройки. От переноса строки они не спасают: `size-adjust` выравнивает
     * вертикальные метрики, а не ширину символов. Замерено 25 августа:
     * настоящий Nunito даёт 182px на «Подобрали для вас», подогнанная грань
     * поверх Roboto — 221px. Подробности в docs/HANDOFF.md.
     */
  },
  ...
  nitro: {
    routeRules: {
      ...
      /*
       * Главная — `isr`, а не `swr`, и это не косметика.
       *
       * `swr: N` vercel-пресет превращает в `{"expiration": false}`: кеш живёт
       * до следующей выкатки и сам не обновляется. Замерено 23 августа —
       * главная отдавалась с `age: 72456`, двадцать часов, причём выкатка
       * счётчик не сбросила.
       *
       * Стоило это битой картинки на витрине. Категории попадают в payload
       * страницы через `useAsyncData('home-ssr-critical')`, а при замене
       * картинки загрузчик удаляет прежний файл — замороженный payload
       * продолжал ссылаться на несуществующий. Сброс кеша из админки
       * (`/api/categories/invalidate`) этого не лечит: он чистит выборку, а
       * не payload уже отрендеренной страницы.
       *
       * Почему здесь безопасно то, что дважды ломало категории. ISR-маршрут
       * vercel-пресета подменяет query-строку, и функция не видит параметров;
       * на категориях из-за этого отваливались бренд-лендинги и сортировка.
       * Главная параметров не читает вовсе — проверено: ни `pages/index.vue`,
      ...
      '/catalog': { swr: 1800 },
      /*
       * Страницы брендов и линеек. Правила у них не было вовсе, и это стоило
       * дороже всего остального на сайте.
       *
       * Замер по бою 15 сентября 2026, три захода подряд, полный ответ:
       *
       *   /                 237 мс   x-vercel-cache: HIT
       *   /catalog/girls    212 мс   HIT
       *   карточка товара   116 мс   HIT
       *   /brand/cada      2719 мс   MISS   (3718 / 2719 / 4640)
       *   /brand/lego      4983 мс   MISS   (4983 / 5432 / 5109)
       *
       * MISS на КАЖДОМ запросе: страница собиралась заново всякий раз, и
       * отсюда же LCP 7,9-9,1 с на бренд-лендингах, который до сих пор
       * списывали на вёрстку и картинки.
       *
       * Почему здесь безопасен `isr`, который дважды ломал категории.
       * ISR-маршрут vercel-пресета подменяет query-строку — на категориях
       * из-за этого отваливались сортировка и фильтры. Страницы бренда и
       * линейки query НЕ ЧИТАЮТ ВООБЩЕ: фильтры и сортировка живут в
       * состоянии компонента и в ключе TanStack Query, в адрес не попадают
       * (`composables/useBrandPageFilters.ts`), `route.query` в
       * `pages/brand/**` и `components/brand/**` не встречается ни разу.
       * Подменять нечего.
       *
       * Персонального в серверной разметке тоже нет: `useAuthStore` на этих
       * страницах вызывается только внутри обработчиков клика
       * (`BrandLandingSpotlight.vue`), корзина и избранное — клиентские.
       *
       * `isr`, а не `swr`, по той же причине, что у каталога: `swr: N`
       * пресет превращает в `{"expiration": false}` — кеш живёт до следующей
       ...
       *
       * 1. `swr: N` vercel-пресет превращает в `{"expiration": false}` (см.
       *    комментарий у главной): кеш не обновлялся по сроку вовсе. Замер по
       *    бою 19 сентября 2026 — живая карточка с `age: 64291`, почти
       *    восемнадцать часов вместо задуманного часа. Цена и наличие
       *    доезжали до покупателя, когда кеш вытеснялся сам.
       *
       * 2. Явный заголовок `public, max-age=3600, stale-while-revalidate=86400`
       *    пересиливал всё, в том числе ответ с ошибкой: минутный сбой базы
       *    давал 503, который браузер держал час, а с stale-while-revalidate
       *    показывал ещё сутки. Перебить заголовок в плагине и в странице не
       *    вышло — проверено тремя способами (docs/HANDOFF.md).
       *
       * С `isr` браузер получает `max-age=0, must-revalidate` — ошибку он не
       * запоминает, — а кешем на сервере управляет Vercel и обновляет
       * страницу раз в час. Подмена query-строки ISR-маршрутом, ломавшая
       * сортировку в каталоге, здесь не мешает: карточка `route.query` не
       * читает.
       */
      '/catalog/products/**': { isr: 3600 },
