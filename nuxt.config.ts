...
export default defineNuxtConfig({
  ...
  nitro: {
    routeRules: {
       ...
       * уходит распроданное.
       *
       * `allowQuery` — почему он тут появился. Бренд из query ушёл, но в нём
       * остались ФИЛЬТРЫ, и они ломались ровно так же. Замер на превью:
       * `/catalog/boys`, `?sort_by=price_asc` и `?sort_by=price_desc` отдавали
       * побайтово один и тот же порядок товаров, а та же сборка локально (без
       * ISR) сортировала правильно — `price_asc` начинался с вертолёта за
       * минимум, `price_desc` с трека Hot Wheels. Клиент это не спасал: он
       * честно слал первый запрос с `p_sort_by=price_desc`, но сетка
       * оставалась серверной, нефильтрованной.
       *
       * Список задаёт, какие параметры платформа пробрасывает в функцию и
       * учитывает в ключе кеша; неперечисленные срезаются. Отсюда известное
       * ограничение: `attr_<слаг>` у атрибутивных фильтров генерируются по
       * данным категории, перечислить их нельзя, и по прямой ссылке с таким
       * параметром выдача останется нефильтрованной. Внутри сайта фильтр
       * работает — там переход клиентский, сервера он не касается.
       *
       ...
       * этом уже ошибались. Для фильтров проверка отдельная: `?sort_by=price_asc`
       * и `?sort_by=price_desc` обязаны давать РАЗНЫЙ первый товар в разметке.
       */
      '/catalog/**': {
        isr: {
          expiration: 1800,
