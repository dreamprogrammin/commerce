...
/*
 * Закрытые для обхода пути — ОДИН список на все группы robots.txt.
 *
 * Раньше у каждой группы был свой: у ИИ-ботов не хватало /login,
 * /register, /forgot-password, /reset-password, а у Яндекса не было ни
 * одного пути вовсе (см. комментарий у его группы). Три копии одного списка
 * расходились молча — поэтому теперь он один.
 */
  ...
  robots: {
    ...
    groups: isPreview
      ...
      : [
          ...
          /*
           * Яндекс — со ВСЕМИ запретами, а не только с Clean-param.
           *
           * Здесь стояла группа из одних `cleanParam`. По правилам Яндекса
           * робот, для которого в файле есть своя группа, читает ТОЛЬКО её и
           * общую `*` пропускает целиком. То есть для Яндекса не действовал
           * ни один запрет: /admin, /login, /order, /profile, /cart, /api/**.
           * Нашёл аудит 24 сентября 2026 (агент ИИ-поиска; технический агент
           * ту же группу записал в «в порядке» — отсюда и эта запись).
           *
           * `Clean-param` при этом межсекционная директива, её можно было бы
           * перенести и в `*`; держим свою группу, чтобы намерение было
           * записано явно, и даём ей тот же список путей, что у всех.
           */
          {
            userAgent: ['Yandex'],
            allow: ['/', '/catalog/**', '/brand/**'],
