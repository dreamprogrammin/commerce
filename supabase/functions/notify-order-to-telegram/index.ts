...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const customerName = orderData.profiles?.first_name
    ...
    }) // Указал часовой пояс для Казахстана
    ...
    if (orderData.delivery_method === 'courier' && orderData.delivery_address) {
      const address = orderData.delivery_address as {
        line1: string
        city: string
      }
    ...
    if (!botToken || !chatId)
      throw new Error('Секреты TELEGRAM не установлены.')
