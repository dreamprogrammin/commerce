...
/**
 * Экранирует то — и только то, — что требует Markdown, каким его шлёт этот бот.
 *
 * ЗДЕСЬ БЫЛ НАБОР ДЛЯ MarkdownV2 (`+`, `-`, `.`, `!`, `#`, `=`, `|`, `{`, `}`,
 * `>`, скобки), а все сообщения уходят с `parse_mode: 'Markdown'` — это
 * первая версия разметки. В ней обратный слеш перед такими символами
 * экранированием НЕ СЧИТАЕТСЯ и печатается как есть, поэтому в рабочем чате
 * менеджеры видели:
 *
 *     *Телефон:* \+77771234567
 ...
 *
 * Поймано 3 сентября 2026 на выводе новых команд менеджера, но касается всех
 * сообщений бота — карточек заказа в том числе, то есть держалось это давно.
 *
 * Markdown первой версии придаёт значение четырём символам: `_`, `*`, `[` и
 * `` ` ``. Их и экранируем. Скобки безопасны: сама по себе `(` разметку не
 * открывает — только `[текст](ссылка)`, а `[` уже экранирована.
 *
 * Если когда-нибудь переведёте отправку на `MarkdownV2`, сюда придётся
