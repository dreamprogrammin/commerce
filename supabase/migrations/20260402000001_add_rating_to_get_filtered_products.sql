-- ============================================================================
-- ДОБАВЛЕНИЕ avg_rating и review_count В get_filtered_products
-- ============================================================================
-- Проблема: ProductCard не отображает рейтинг и отзывы, потому что
-- функция get_filtered_products() не возвращает эти поля из таблицы products
...
-- Создаем обновленную версию с полями avg_rating и review_count
...
BEGIN
    ...
    GROUP BY
        ...
        fp.sales_count, fp.is_active, fp.min_age_years, fp.max_age_years, fp.gender, fp.accessory_ids, fp.is_accessory,
        fp.barcode, fp.brand_id, fp.origin_country_id, fp.material_id, fp.discount_percentage, fp.created_at, fp.updated_at,
        -- ⭐ ДОБАВЛЕНО: Рейтинг и отзывы в GROUP BY
...
'Возвращает отфильтрованный список товаров с изображениями, рейтингом и количеством отзывов. Поддерживает фильтр по количеству деталей (piece_count).';
