-- =====================================================================================
-- ИСПРАВЛЕНИЕ: Возврат потраченных бонусов при отмене заказа в статусе NEW
-- =====================================================================================
-- Проблема:
-- - Бонусы списываются при СОЗДАНИИ заказа (create_user_order)
-- - Но откатываются ТОЛЬКО при отмене ПОДТВЕРЖДЕННОГО заказа (bonuses_activation_date IS NOT NULL)
-- - Если отменить заказ в статусе NEW, потраченные бонусы НЕ возвращаются
-- - Пользователь теряет бонусы! ❌
--
-- Решение:
-- - Возвращать bonuses_spent ВСЕГДА (для всех статусов)
-- - Откатывать bonuses_awarded и приветственный бонус ТОЛЬКО для подтвержденных заказов
-- =====================================================================================
...
BEGIN
  ...
  IF v_target_order.user_id IS NOT NULL THEN
    ...
    IF v_user_profile IS NOT NULL THEN
      -- Для заказов в статусе NEW: только возвращаем потраченные бонусы
      IF v_target_order.status IN ('pending', 'new') THEN
        IF COALESCE(v_target_order.bonuses_spent, 0) > 0 THEN
          ...
          RAISE NOTICE 'Возвращены потраченные бонусы % для заказа % (статус: %)',
            v_target_order.bonuses_spent, p_order_id, v_target_order.status;
        END IF;
...
Логика отката:
  ...
  - bonuses_awarded: ТОЛЬКО для confirmed/processing (начисляются при подтверждении)
  - Приветственный бонус: откатывается если это был единственный подтвержденный заказ

ИСПРАВЛЕНИЕ v1.2 (2026-01-05):
  - Потраченные бонусы (bonuses_spent) возвращаются для ВСЕХ статусов, включая new/pending
  - Это исправляет проблему потери бонусов при отмене неподтвержденного заказа';
