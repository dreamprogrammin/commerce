-- =====================================================================================
-- Миграция: Исправление системы активации бонусов (БЕЗОПАСНАЯ ВЕРСИЯ)
...
-- ❌ ОШИБКА: CREATE OR REPLACE VIEW не работает, если меняется структура
-- ✅ ПРАВИЛЬНО: Сначала удаляем, потом создаем заново

DROP VIEW IF EXISTS public.bonus_system_status CASCADE;

-- Создаём функцию для безопасного получения статуса cron
CREATE OR REPLACE FUNCTION public.get_cron_status_safe()
RETURNS TABLE (
...
BEGIN
    ...
    ELSE
        ...
        RETURN QUERY SELECT NULL::TIMESTAMPTZ, NULL::TEXT;
    END IF;
EXCEPTION WHEN OTHERS THEN
    -- Если ошибка (например, таблицы cron не существуют), возвращаем NULL
    RETURN QUERY SELECT NULL::TIMESTAMPTZ, NULL::TEXT;
END;
$$;

COMMENT ON FUNCTION public.get_cron_status_safe IS 
'Безопасно возвращает статус cron задания (не падает, если cron недоступен)';
...
FROM public.get_cron_status_safe() cs;

COMMENT ON VIEW public.bonus_system_status IS 
'Мониторинг состояния бонусной системы (работает с pg_cron и без него)';
