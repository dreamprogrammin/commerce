-- =====================================================================================
-- Исправление ошибки "column reference category_id is ambiguous"
-- =====================================================================================
-- Проблема: В функции get_personalized_recommendations при выборе из CTE
-- last_purchased_category возникает неоднозначность с колонкой category_id
--
-- Решение: Явно указать alias для колонки в CTE и использовать его
-- =====================================================================================
...
BEGIN
    ...
    -- ✅ ИСПРАВЛЕНИЕ: Явно указываем alias для колонки в CTE
    IF EXISTS (SELECT 1 FROM public.orders WHERE user_id = p_user_id) THEN
        ...
        SELECT
            ...
            p.barcode, p.brand_id, p.origin_country_id, p.material_id, p.discount_percentage, p.created_at, p.updated_at,
            (SELECT json_agg(pi) FROM public.product_images pi WHERE pi.product_id = p.id) as product_images
        FROM public.products p, last_purchased_category lpc
...
'Персонализированные рекомендации товаров для пользователя.
Исправлена ошибка с неоднозначностью category_id в CTE.';
