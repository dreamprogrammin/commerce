-- =====================================================================================
-- ИСПРАВЛЕНИЕ: Откат bonuses_awarded для ВСЕХ статусов при отмене заказа
-- =====================================================================================
-- Проблема:
-- - bonuses_awarded начисляются в pending_bonus_balance при СОЗДАНИИ заказа
-- - Но откатываются только для статусов 'confirmed'/'processing'
-- - При отмене заказа в статусе 'new'/'pending' бонусы остаются на счету ❌
--
-- Решение:
-- - Откатывать bonuses_awarded из pending_bonus_balance для ВСЕХ статусов
-- - Списываются при создании → должны откатываться при отмене
-- =====================================================================================
...
BEGIN
  ...
  -- ✅ ИСПРАВЛЕНИЕ: Откатываем бонусы для ВСЕХ статусов
  IF v_target_order.user_id IS NOT NULL THEN
    ...
    IF v_user_profile IS NOT NULL THEN
      -- ✅ Для заказов в статусе NEW/PENDING: откатываем spent + awarded
      IF v_target_order.status IN ('pending', 'new') THEN
        UPDATE public.profiles
        SET
          -- Возвращаем потраченные бонусы
          active_bonus_balance = active_bonus_balance + COALESCE(v_target_order.bonuses_spent, 0),
          -- ✅ ИСПРАВЛЕНИЕ: Убираем начисленные бонусы (они были добавлены при создании)
          pending_bonus_balance = GREATEST(pending_bonus_balance - COALESCE(v_target_order.bonuses_awarded, 0), 0)
        WHERE id = v_target_order.user_id;
      ...
      -- Для подтвержденных заказов: полный откат (spent + awarded + welcome)
      ELSIF v_target_order.status IN ('confirmed', 'processing') THEN
        ...
        BEGIN
          ...
          IF v_other_confirmed_orders = 0 AND v_user_profile.has_received_welcome_bonus THEN
            ...
            UPDATE public.profiles
            SET
              -- Возвращаем потраченные бонусы, убираем приветственный
...
Логика отката:
  ...
  - bonuses_spent: ВСЕГДА возвращаются (для всех статусов)
  - bonuses_awarded: ВСЕГДА откатываются из pending_bonus_balance (для всех статусов)
  - Приветственный бонус: откатывается если это был единственный подтвержденный заказ

ИСПРАВЛЕНИЕ v1.3 (2026-01-05):
  - bonuses_awarded откатываются для ВСЕХ статусов (не только confirmed/processing)
  - Это исправляет проблему с оставшимися бонусами в pending при отмене нового заказа';
