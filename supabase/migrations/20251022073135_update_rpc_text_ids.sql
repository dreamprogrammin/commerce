-- Up Migration: Унификация ID фильтров (Country, Material) до TEXT[]

-- 2. Теперь создаем финальную, правильную версию с аргументами TEXT[] и логикой WHERE
CREATE OR REPLACE FUNCTION public.get_filtered_products(
    ...
    -- === ИЗМЕНЕННЫЕ АРГУМЕНТЫ (TEXT[] для унификации на фронтенде) ===
...
BEGIN
    ...
    WHERE
        ...
        -- 2. Фильтр по брендам (UUID[] -> работает как TEXT[])
        AND (p_brand_ids IS NULL OR CARDINALITY(p_brand_ids) = 0 OR p.brand_id = ANY(p_brand_ids))
        -- 3. Фильтр по цене
        AND (p_price_min IS NULL OR p.price >= p_price_min)
        AND (p_price_max IS NULL OR p.price <= p_price_max)

        -- === 4. НОВЫЙ ФИЛЬТР: Страна происхождения (Конвертация TEXT[] -> INTEGER[]) ===
        ...
        -- === 5. НОВЫЙ ФИЛЬТР: Материал (Конвертация TEXT[] -> INTEGER[]) ===
