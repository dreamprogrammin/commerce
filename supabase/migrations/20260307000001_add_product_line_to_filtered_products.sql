-- Добавляем product_line_id и product_line_name в результат get_filtered_products
DROP FUNCTION IF EXISTS public.get_filtered_products(
    TEXT, UUID[], TEXT[], NUMERIC, NUMERIC, TEXT, INT, INT,
    public.attribute_filter[], TEXT[], TEXT[], TEXT[], INT, INT,
    public.numeric_attribute_filter[]
);
...
BEGIN
    ...
    GROUP BY
        ...
        fp.sales_count, fp.is_active, fp.min_age_years, fp.max_age_years, fp.gender, fp.accessory_ids, fp.is_accessory,
        fp.barcode, fp.brand_id, fp.origin_country_id, fp.material_id, fp.discount_percentage, fp.created_at, fp.updated_at,
        fp.brand_name, fp.brand_slug, fp.product_line_id, fp.product_line_name, fp.sort_order
...
COMMENT ON FUNCTION public.get_filtered_products(
    ...
    public.attribute_filter[], TEXT[], TEXT[], TEXT[], INT, INT,
    public.numeric_attribute_filter[]
) IS 'Returns filtered products with product line info, images, and brand data';
