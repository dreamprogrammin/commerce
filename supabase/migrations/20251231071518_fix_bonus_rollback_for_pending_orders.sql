-- =====================================================================================
-- Исправление отката бонусов для заказов в статусе 'pending'
-- =====================================================================================
-- Проблема:
-- - При создании заказа бонусы начисляются/списываются СРАЗУ (независимо от статуса)
-- - При отмене заказа бонусы откатываются ТОЛЬКО для статусов confirmed/processing
-- - Если заказ отменяется в статусе pending - бонусы НЕ откатываются → "застревают"
--
-- Решение:
-- - Возврат товаров на склад - ТОЛЬКО для confirmed/processing (товары списываются при подтверждении)
-- - Откат бонусов - для ВСЕХ статусов (бонусы начисляются при создании заказа)
-- =====================================================================================
...
BEGIN
  ...
  -- Проверяем статус (поддерживаем pending, confirmed, processing)
  IF v_target_order.status NOT IN ('pending', 'confirmed', 'processing') THEN
    RETURN 'Ошибка: Этот заказ уже нельзя отменить (возможно, он выполнен или уже отменен).';
  END IF;

  -- ✅ ИСПРАВЛЕНИЕ: Возврат товаров ТОЛЬКО для confirmed/processing
  ...
  -- ✅ ИСПРАВЛЕНИЕ: Откат бонусов для ВСЕХ статусов (pending/confirmed/processing)
  -- (бонусы начисляются/списываются СРАЗУ при создании заказа, независимо от статуса)
...
Поддерживает статусы: pending, confirmed, processing.

Логика отката:
  - Товары на склад: ТОЛЬКО для confirmed/processing (товары списываются при подтверждении)
  - Бонусы: для ВСЕХ статусов (бонусы начисляются при создании заказа)
  - Приветственный бонус откатывается если это был единственный заказ';
