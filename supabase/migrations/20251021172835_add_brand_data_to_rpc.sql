-- Up Migration: Добавляем данные о бренде в RPC-функцию get_filtered_products
...
RETURNS TABLE (
    ...
    -- Дополнительные поля
    product_images JSON,
    brand_name TEXT, -- <-- НОВОЕ ПОЛЕ
    brand_slug TEXT  -- <-- НОВОЕ ПОЛЕ
)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    SELECT
        ...
        -- ==== ИЗМЕНЕНИЕ ЗДЕСЬ: Получаем имя и слаг из присоединенной таблицы `brands` ====
        b.name AS brand_name,
        b.slug AS brand_slug
    FROM
        public.products p
    -- ==== ИЗМЕНЕНИЕ ЗДЕСЬ: Используем LEFT JOIN, чтобы не потерять товары без бренда ====
    LEFT JOIN
