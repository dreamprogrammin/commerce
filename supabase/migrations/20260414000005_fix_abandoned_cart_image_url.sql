-- Исправляем URL изображений в функции брошенной корзины
...
BEGIN
  ...
  LOOP
    ...
    IF v_items_count = 1 THEN
      ...
      v_body := 'В вашей корзине: ' || COALESCE(v_product.name, 'товар') || E'\n\nСумма: ' || r.total_amount || ' ₸' || E'\nЗавершите покупку на uhti.kz';
      
      IF v_product.image_url IS NOT NULL THEN
        v_photos := jsonb_build_array(
          jsonb_build_object('url', 
            'https://gvsdevsvzgcivpphcuai.supabase.co/storage/v1/object/public/' || v_product.image_url
          )
        );
      END IF;
    ...
    ELSIF v_items_count BETWEEN 2 AND 3 THEN
      ...
      LOOP
        ...
        v_product_names := v_product_names || '• ' || COALESCE(v_product.name, 'товар') || '\n';
      END LOOP;
      
      v_body := 'В вашей корзине ' || v_items_count || ' товара:' || E'\n\n' || 
                v_product_names || E'\nСумма: ' || r.total_amount || ' ₸' || E'\nЗавершите покупку на uhti.kz';
  ...
  -- 30 минут напоминание (упрощенное)
  FOR r IN
    SELECT sc.id, sc.user_id, sc.items, sc.total_amount, p.telegram_chat_id
    FROM server_carts sc
    JOIN profiles p ON p.id = sc.user_id
    WHERE sc.updated_at < now() - interval '30 minutes'
  ...
  LOOP
    ...
    v_body := 'Ваши товары всё ещё ждут вас! Сумма: ' || r.total_amount || ' ₸' || E'\nЗавершите покупку на uhti.kz';
