-- ============================================================================
-- ФИКС: Фильтр и диапазон цен теперь учитывают скидку (discount_percentage)
-- ============================================================================
--
-- ПРОБЛЕМА:
-- get_category_price_range() и get_filtered_products() использовали p.price
-- (оригинальную цену) для определения диапазона и фильтрации.
-- Товары со скидкой не попадали в фильтр по цене.
--
-- РЕШЕНИЕ:
-- Используем эффективную цену: p.price * (100 - COALESCE(p.discount_percentage, 0)) / 100
-- ============================================================================

-- ============================================================================
-- 1. ФИКС get_category_price_range — min/max по цене со скидкой
-- ============================================================================
...
BEGIN
    ...
    -- Возвращаем минимальную и максимальную цену с учётом скидки
    RETURN QUERY
    SELECT
...
-- ============================================================================
-- 2. ФИКС get_filtered_products — фильтрация и сортировка по цене со скидкой
-- ============================================================================
...
BEGIN
    ...
    WITH filtered_products AS (
        SELECT
            ...
            -- Вычисляем эффективную цену один раз для использования в сортировке
        ...
        WHERE
            ...
            -- 4. Фильтр по цене (с учётом скидки)
        ...
        ORDER BY
            ...
            CASE p_sort_by
                WHEN 'price_asc' THEN (p.price * (100 - COALESCE(p.discount_percentage, 0)) / 100) END ASC NULLS LAST,
            CASE p_sort_by
                WHEN 'price_desc' THEN (p.price * (100 - COALESCE(p.discount_percentage, 0)) / 100) END DESC NULLS LAST,
            p.name ASC
        LIMIT p_page_size
        OFFSET v_offset
    )
    ...
    GROUP BY
        ...
        fp.sales_count, fp.is_active, fp.min_age_years, fp.max_age_years, fp.gender, fp.accessory_ids, fp.is_accessory,
        fp.barcode, fp.brand_id, fp.origin_country_id, fp.material_id, fp.discount_percentage, fp.created_at, fp.updated_at,
        fp.brand_name, fp.brand_slug, fp.effective_price
    ORDER BY
        CASE p_sort_by
        ...
        CASE p_sort_by
            WHEN 'price_asc' THEN fp.effective_price END ASC NULLS LAST,
        CASE p_sort_by
            WHEN 'price_desc' THEN fp.effective_price END DESC NULLS LAST,
        fp.name ASC;
END;
$$;
...
'Возвращает отфильтрованный список товаров с изображениями. Фильтрация и сортировка по цене учитывают discount_percentage.';
