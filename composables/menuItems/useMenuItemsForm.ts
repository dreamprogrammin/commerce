...
export function useMenuItemFormData (initialSelectItem : Ref<MenuItemRow | null>) {
    ...
    function removeChild(indexOrTempId: number | string):void {
        if (typeof indexOrTempId === 'number') {
            ...
            childrenForms.value = childrenForms.value.filter(childrenForm => childrenForm._tempId !== indexOrTempId)
        }
    }
    ...
    function handleItemChange(event:Event, itemForm: IEditableMenuItem):void {
        ...
        if (target.files && target.files[0]) {
            ...
            itemForm._imagePreviewUrl = getStoragePublicUrl(BUCKET_NAME, itemForm.image_url || null)
        }
    ...
    //         toast.error('Валидация основного пункта', { description: 'Заголовок и Слаг обязательны!' });
    //         return false;
    //     }

    //     for (const child of childrenForms.value) {
    //         if (!child.title?.trim() || !child.slug?.trim()) {
    //             toast.error('Валидация подпункта', { description: `Заголовок и Слаг для "${child.title || 'Новый подпункт'}" обязательны!` });
    //             return false;
    //         }
    //     }
    ...
    //     try {
    //         let parentSlugForChildren = form.value.slug!
    //         let parentId = form.value.id
    ...
    //                 filePathPrefix: `menu/${form.value.slug}`
    //             })
    //             if (!path) {
    ...
    //         const mainItemPayload :Partial<MenuItemInsert | MenuItemUpdate> = {...form.value}
    //         delete mainItemPayload._tempId
    //     } catch {
    ...
    //     for (const child of childrenForms.value) {
    //         if (!child.title?.trim() && !child.slug?.trim()) {
    //             toast.error('Валидация подпункта', {
    //                 description: `Заголовок и Слаг для подпункта "${child.title || 'Новый подпункт без имени'}" обязательны!`
    //             })
    //             return false
    //         }
    //     }

    //     menuItemsStore.isLoading = true

    //     try {
    //         let parentSlugForChildren = form.value.slug!
    //         let parentId = form.value.id
    //         let mainItemFinalImageUrl = form.value.image_url || null
    ...
    //             const path = await uploadStorageFile(form.value._imageFile, {
    //                 bucketName:BUCKET_NAME,
    //                 filePathPrefix: `menu/${form.value.slug || 'new_item'}`
    //             })

    //             if (!path) {
    //                 menuItemsStore.isLoading = false
    //                 return false
    //             }

    //             mainItemFinalImageUrl = path
    //         } else if (form.value.image_url === null && initialSelectItem.value?.image_url) {
    //             mainItemFinalImageUrl = null
    //         }


    //         let mainItemType = form.value.item_type || 'link'
    //         if (childrenForms.value.length > 0) {
    //             mainItemType = (form.value.href && form.value.href.trim() !== '') ? 'trigger_and_link' : 'trigger'
    //         } else {
    //             mainItemType = (form.value.href?.trim() && form.value.href.trim() !== '') ? 'link' : (mainItemType === 'trigger' || mainItemType === 'trigger_and_link' ? mainItemType : 'link')
    //         }

    //         const mainItemDataForDb: Partial<IEditableMenuItem> = {
    //             ...form.value,
    //             image_url:mainItemFinalImageUrl,
    //             item_type:mainItemType as Database['public']['Enums']['menu_item_type_enum'],
    //             href: form.value.href?.trim() === '' ? null : (form.value.href || null),
    //             description: form.value.description?.trim() === '' ? null : (form.value.description || null),
    //             parent_slug: form.value.parent_slug === NO_PARENT_OPTION_NAME ? null : (form.value.parent_slug?.trim() === '' ? null : (form.value.parent_slug || null)),
    //             icon_name: form.value.icon_name?.trim() === '' ? null : (form.value.icon_name || null),
    //             display_order: form.value.display_order || 0
    //         }

    //         delete mainItemDataForDb._tempId
    ...
    async function submit(): Promise<boolean> {
        ...
        if (!form.value.title?.trim() || !form.value.slug?.trim()) {
            toast.error('Ошибка валидации основного пункта', { description: 'Заголовок и Слаг обязательны!' });
            isProcessing.value = false;
            return false;
        }
        if (!initialSelectItem.value && !form.value.slug?.trim()) {
            toast.error('Ошибка валидации основного пункта', { description: 'Слаг обязателен для нового пункта!' });
            isProcessing.value = false;
            return false;
        }
        ...
        for (const child of childrenForms.value) {
            if (!child.title?.trim() || !child.slug?.trim()) {
                toast.error('Ошибка валидации подпункта', { description: `Заголовок и Слаг для "${child.title || 'Новый подпункт'}" обязательны!` });
                isProcessing.value = false;
                return false;
            }
        }
        ...
        try {
            ...
            let mainItemTypeCalculated: Database['public']['Enums']['menu_item_type_enum'] = form.value.item_type || 'link'
            if (childrenForms.value.length > 0) {
                mainItemTypeCalculated = (form.value.href && form.value.href?.trim() !== '') ? 'trigger_and_link' : 'trigger'
            } else {
                mainItemTypeCalculated = (form.value.href && form.value.href.trim() !== '') ? 'link' : 'trigger'

                if (!form.value.href || form.value.href.trim() === '') {
                    if (form.value.item_type === 'trigger' || form.value.item_type === 'trigger_and_link') {
            ...
            const mainItemPayload: Partial<MenuItemInsert | MenuItemUpdate> = {
                ...
                parent_slug: form.value.parent_slug === NO_PARENT_OPTION_NAME ? null : form.value.parent_slug,
                display_order: form.value.display_order || 0,
                image_url: finalMainImageUrl,
                icon_name: form.value.icon_name?.trim() ? form.value.icon_name.trim() : null,
            }

            let savedMainItem: MenuItemRow | null = null
            
            if (initialSelectItem.value?.id) {
                savedMainItem = await menuItemsStore.updatedItem(initialSelectItem.value.id, mainItemPayload as MenuItemUpdate)
            } else {
                if (!form.value.slug) throw new Error('Slug is required for new main item');
                (mainItemPayload as MenuItemInsert).slug = form.value.slug
                savedMainItem = await menuItemsStore.addItem(mainItemPayload as MenuItemInsert)
            }
            ...
            const existingDbChildren = initialSelectItem.value ? menuItemsStore.getChildren(initialSelectItem.value.slug) : []
            const childrenInFormIdsAndTempIds = new Set(childrenForms.value.map(cf => cf.id || cf._tempId))
            ...
            for (const childForm of childrenForms.value) {
                ...
                } else if (childForm.image_url === null && existingDbChildren.find(c => c.id === childForm.id)?.image_url) {
                    const oldChildData = existingDbChildren.find(c => c.id === childForm.id)
                    if (oldChildData?.image_url) await removeStorageFile(BUCKET_NAME, oldChildData.image_url);
                    finalChildImageUrl = null
                }


                const childItemType: Database['public']['Enums']['menu_item_type_enum'] = (childForm.href && childForm.href.trim() !== '') ? 'link' : 'trigger_and_link'
                ...
                const childDbPayload : Partial<MenuItemInsert | MenuItemUpdate> = {
                    ...
                    description: childForm.description?.trim() ? childForm.description?.trim() : null,
                    item_type: childItemType,
                    parent_slug: savedMainItem.slug,
                    display_order: childForm.display_order || 0,
                    image_url: finalChildImageUrl,
                    icon_name: childForm.icon_name?.trim() ? childForm.icon_name.trim() : null,
                }

                if (childForm.id && !childForm._tempId) {
                    await menuItemsStore.updatedItem(childForm.id, childDbPayload as MenuItemUpdate)
                } else {
                    if (!childForm.slug) throw new Error("Slug is required for new child item");
