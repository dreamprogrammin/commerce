...
const storeSchema = {
  ...
  'priceRange': '₸₸',
  /*
   * Способы оплаты — слово в слово те же, что у узла организации в `app.vue`.
   * Стояло `['Наличные', 'Карты', 'Каспи']`, и «Карты» пережили 16 сентября,
   * когда оплату картой убрали с оформления: соседний узел тогда поправили, а
   * этот пропустили, и разметка обещала способ, которого на сайте нет.
   */
  ...
  /*
   * КООРДИНАТ ЗДЕСЬ НЕТ НАМЕРЕННО.
   *
   * Стояли 43.222, 76.8512 — и это другой конец города: обратное
   * геокодирование даёт «улица Айриха, Ауэзовский район», а адрес строкой
   * выше называет мкр. Шапагат в Алатауском районе. Между точками 7,3 км.
   *
   * Адрес верный (проверено 16 сентября 2026: OSM знает «Аманкелді көшесі
   * (Шапағат), Алатау ауданы, Алматы»), а точка была неверной. Решение
   * владельца — убрать координаты, а не угадывать: ошибочная точка на карте
   * хуже, чем её отсутствие, и для покупателя, и для локального поиска.
   *
   * Когда адрес подтвердят с номером дома, координаты вернутся сюда же.
   * Ориентир по OSM для улицы из адреса: 43.2843, 76.8806.
   */
  'areaServed': {
    '@type': 'City',
    'name': 'Алматы',
  ...
  /*
   * Связь с узлом организации из `app.vue`. Без неё в разметке главной лежали
   * ДВЕ сущности с одним адресом и телефоном: у организации почта и профили в
   * соцсетях, у магазина часы работы, — и поисковику приходилось догадываться,
   * что это одно и то же. Ссылка односторонняя намеренно: `#store` живёт
   * только на главной, а `#organization` — на каждой странице, и обратная
   * ссылка указывала бы в пустоту на всех остальных.
   */
