...
function handleAddChild(parentItem: IEditableMenuItem | { children: IEditableMenuItem[], slug: string }) {
  ...
  if (!parentItem.slug) {
    toast.error('Невозможно добавить подпункт', {
      description: 'Родительский пункт должен быть сначала сохранен (чтобы получить слаг).',
    });
    return;
  }
...
<template>
  <div class="container mx-auto p-4 md:p-8">
    <div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
      <h1 class="text-3xl font-bold text-foreground">Управление содержимым меню</h1>
      <Button v-if="selectedParent" @click="saveAllChanges" :disabled="isSaving">
    ...
    <div v-else-if="menuAdminStore.storeError" class="my-6 p-4 bg-destructive/10 text-destructive border rounded-md shadow">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-4 gap-8">
      ...
      <aside class="lg:col-span-1">
        ...
        <div class="space-y-1">
          <Button
            v-for="staticParent in staticMainMenuItems.filter(i => i.isTrigger)"
            :key="staticParent.slug"
            @click="selectParent(staticParent)"
            variant="ghost"
            class="w-full justify-start text-base h-auto py-2 text-left"
            :class="{'bg-accent text-accent-foreground font-semibold': selectedParent?.slug === staticParent.slug}"
          >
            <Icon v-if="staticParent.iconName" :name="staticParent.iconName" class="mr-2 h-4 w-4 flex-shrink-0" />
      ...
      <main class="lg:col-span-3">
        <div v-if="selectedParent" class="bg-card p-6 rounded-lg shadow-md space-y-4">
          <h2 class="text-xl font-bold">
            Редактирование подменю для: <span class="text-primary">{{ selectedParent.title }}</span>
          </h2>
          
          <div v-if="formTree.length === 0" class="text-muted-foreground text-center py-5 border-dashed border-2 rounded-md">
          ...
          <div v-for="(itemL2, index) in formTree" :key="itemL2.id || itemL2._tempId!">
          ...
          <div class="pt-4 border-t">
            <Button @click="addTopLevelNode" variant="outline" class="w-full border-dashed">
                <Icon name="lucide:plus" class="mr-2"/> Добавить пункт в "{{ selectedParent.title }}" (Уровень 2)
            </Button>
          </div>
        </div>
