<script setup lang="ts">
import { Package, Calendar, CreditCard, MapPin, Gift, XCircle } from 'lucide-vue-next'
import { toast } from 'vue-sonner'

const route = useRoute()
const router = useRouter()
const supabase = useSupabaseClient()
const user = useSupabaseUser()

const orderId = route.params.id as string

// Состояние
const order = ref<any | null>(null)
const isLoading = ref(true)
const error = ref<string | null>(null)
const needsVerification = ref(false)
const verificationEmail = ref('')
const isVerifying = ref(false)
const isCancelling = ref(false)
const showCancelDialog = ref(false)

// Определяем тип заказа (orders или guest_checkouts)
const orderTableName = ref<'orders' | 'guest_checkouts'>('orders')

// Загрузка заказа
const fetchOrder = async (skipVerification = false) => {
  isLoading.value = true
  error.value = null

  try {
    // Если пользователь авторизован, пытаемся загрузить из orders
    if (user.value) {
      const { data, error: fetchError } = await supabase
        .from('orders')
        .select(`
          id,
          created_at,
          status,
          final_amount,
          delivery_method,
          payment_method,
          delivery_address,
          bonuses_spent,
          bonuses_awarded,
          order_items(
            id,
            quantity,
            product:products(
              id,
              name,
              price,
              product_images(
                image_url,
                blur_placeholder
              )
            )
          )
        `)
        .eq('id', orderId)
        .eq('user_id', user.value.id)
        .single()

      if (!fetchError && data) {
        order.value = data
        orderTableName.value = 'orders'
        needsVerification.value = false
        isLoading.value = false
        return
      }
    }

    // Если не авторизован или заказ не найден, пытаемся загрузить из guest_checkouts
    // Но сначала нужна верификация
    if (!skipVerification) {
      needsVerification.value = true
      isLoading.value = false
      return
    }

    // Загружаем гостевой заказ с проверкой email
    const { data: guestData, error: guestError } = await supabase
      .from('guest_checkouts')
      .select(`
        id,
        created_at,
        status,
        final_amount,
        delivery_method,
        payment_method,
        delivery_address,
        guest_name,
        guest_phone,
        guest_email,
        guest_checkout_items(
          id,
          quantity,
          price_per_item,
          product:products(
            id,
            name,
            price,
            product_images(
              image_url,
              blur_placeholder
            )
          )
        )
      `)
      .eq('id', orderId)
      .eq('guest_email', verificationEmail.value)
      .single()

    if (guestError) {
      if (guestError.code === 'PGRST116') {
        error.value = 'Заказ не найден или неверный email'
      } else {
        throw guestError
      }
      needsVerification.value = true
      return
    }

    // Преобразуем структуру гостевого заказа
    order.value = {
      ...guestData,
      bonuses_spent: 0,
      bonuses_awarded: 0,
      order_items: guestData.guest_checkout_items.map((item: any) => ({
        id: item.id,
        quantity: item.quantity,
        product: {
          ...item.product,
          price: item.price_per_item,
        },
      })),
    }
    orderTableName.value = 'guest_checkouts'
    needsVerification.value = false
  } catch (err: any) {
    console.error('Ошибка загрузки заказа:', err)
    error.value = err.message
  } finally {
    isLoading.value = false
  }
}

// Верификация email для гостевого заказа
const verifyEmail = async () => {
  if (!verificationEmail.value) {
    toast.error('Введите email')
    return
  }

  isVerifying.value = true
  await fetchOrder(true)
  isVerifying.value = false
}

// Отмена заказа
const handleCancelOrder = async () => {
  if (!order.value) return

  isCancelling.value = true

  try {
    const { data, error: cancelError } = await supabase.rpc('cancel_order', {
      p_order_id: order.value.id,
      p_table_name: orderTableName.value,
    })

    if (cancelError) {
      console.error('Ошибка отмены заказа:', cancelError)
      toast.error('Не удалось отменить заказ', {
        description: cancelError.message,
      })
      return
    }

    // Обновляем заказ
    await fetchOrder(orderTableName.value === 'guest_checkouts')
    showCancelDialog.value = false

    toast.success('Заказ успешно отменён')
  } catch (err: any) {
    console.error('Критическая ошибка при отмене:', err)
    toast.error('Произошла ошибка', {
      description: err.message || 'Попробуйте позже',
    })
  } finally {
    isCancelling.value = false
  }
}

// Проверить можно ли отменить заказ
const canCancelOrder = computed(() => {
  if (!order.value) return false
  return order.value.status === 'new' || order.value.status === 'confirmed'
})

// Форматирование адреса
const formatAddress = (address: any) => {
  if (!address) return 'Не указан'
  if (typeof address === 'string') return address

  const parts = []
  if (address.city) parts.push(address.city)
  if (address.line1) parts.push(address.line1)
  if (address.street) parts.push(address.street)
  if (address.building) parts.push(`д. ${address.building}`)
  if (address.apartment) parts.push(`кв. ${address.apartment}`)

  return parts.length > 0 ? parts.join(', ') : 'Не указан'
}

// Метод доставки
const deliveryMethodLabel = computed(() => {
  if (!order.value) return ''
  const method = order.value.delivery_method
  return method === 'delivery' ? 'Доставка курьером' : 'Самовывоз'
})

// Метод оплаты
const paymentMethodLabel = computed(() => {
  if (!order.value?.payment_method) return 'Не указан'
  const method = order.value.payment_method
  const labels: Record<string, string> = {
    cash: 'Наличными',
    card: 'Картой',
    kaspi: 'Каспи',
  }
  return labels[method] || method
})

// Цвет статуса
const getStatusColor = (status: string) => {
  const colors: Record<string, string> = {
    new: 'bg-blue-100 text-blue-800',
    confirmed: 'bg-green-100 text-green-800',
    processing: 'bg-yellow-100 text-yellow-800',
    delivered: 'bg-purple-100 text-purple-800',
    cancelled: 'bg-red-100 text-red-800',
  }
  return colors[status] || 'bg-gray-100 text-gray-800'
}

// Текст статуса
const getStatusLabel = (status: string) => {
  const labels: Record<string, string> = {
    new: 'Новый заказ',
    confirmed: 'Подтверждён',
    processing: 'В обработке',
    delivered: 'Доставлен',
    cancelled: 'Отменён',
  }
  return labels[status] || status
}

// Инициализация
onMounted(() => {
  // Проверяем sessionStorage для гостей
  if (typeof window !== 'undefined') {
    const savedEmail = sessionStorage.getItem('guest_order_email')
    if (savedEmail) {
      verificationEmail.value = savedEmail
      // Автоматически верифицируем если есть email
      fetchOrder(true)
      // Очищаем после использования
      sessionStorage.removeItem('guest_order_email')
      return
    }
  }

  fetchOrder()
})

// Мета
definePageMeta({
  layout: 'default',
})

useHead({
  title: `Заказ №${orderId.slice(-6)}`,
})
</script>

<template>
  <div class="container max-w-4xl mx-auto py-8 px-4">
    <!-- Загрузка -->
    <div v-if="isLoading" class="space-y-4">
      <Skeleton class="h-10 w-1/2" />
      <Skeleton class="h-64 w-full" />
      <Skeleton class="h-48 w-full" />
    </div>

    <!-- Форма верификации для гостей -->
    <Card v-else-if="needsVerification" class="max-w-md mx-auto">
      <CardHeader>
        <CardTitle>Подтверждение доступа</CardTitle>
        <CardDescription>
          Для просмотра заказа №{{ orderId.slice(-6) }} введите email, указанный при оформлении
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form @submit.prevent="verifyEmail" class="space-y-4">
          <div>
            <Label for="email">Email</Label>
            <Input
              id="email"
              v-model="verificationEmail"
              type="email"
              placeholder="your@email.com"
              required
            />
          </div>
          <Button type="submit" class="w-full" :disabled="isVerifying">
            <span v-if="isVerifying">Проверка...</span>
            <span v-else>Показать заказ</span>
          </Button>
        </form>
        <p v-if="error" class="text-sm text-destructive mt-4">
          {{ error }}
        </p>
      </CardContent>
    </Card>

    <!-- Ошибка -->
    <div v-else-if="error" class="text-center py-12 space-y-4">
      <Icon name="lucide:alert-circle" class="w-16 h-16 text-destructive mx-auto" />
      <h2 class="text-xl font-semibold">Заказ не найден</h2>
      <p class="text-muted-foreground">{{ error }}</p>
      <Button @click="router.push('/')">
        На главную
      </Button>
    </div>

    <!-- Содержимое заказа -->
    <div v-else-if="order" class="space-y-6">
      <!-- Заголовок -->
      <div class="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 class="text-2xl md:text-3xl font-bold mb-2">
            Заказ №{{ order.id.slice(-6) }}
          </h1>
          <div class="flex items-center gap-2 text-sm text-muted-foreground">
            <Calendar class="w-4 h-4" />
            <span>{{ new Date(order.created_at).toLocaleString('ru-RU') }}</span>
          </div>
        </div>
        <Badge :class="getStatusColor(order.status)" class="text-base px-4 py-2">
          {{ getStatusLabel(order.status) }}
        </Badge>
      </div>

      <!-- Информация о заказе -->
      <div class="grid md:grid-cols-2 gap-6">
        <!-- Детали заказа -->
        <Card>
          <CardHeader>
            <CardTitle>Детали заказа</CardTitle>
          </CardHeader>
          <CardContent class="space-y-4">
            <!-- Доставка -->
            <div class="flex items-start gap-3">
              <Package class="w-5 h-5 text-muted-foreground mt-0.5" />
              <div class="flex-1">
                <p class="text-sm font-medium">Способ доставки</p>
                <p class="text-sm text-muted-foreground">{{ deliveryMethodLabel }}</p>
              </div>
            </div>

            <!-- Адрес -->
            <div v-if="order.delivery_method === 'delivery'" class="flex items-start gap-3">
              <MapPin class="w-5 h-5 text-muted-foreground mt-0.5" />
              <div class="flex-1">
                <p class="text-sm font-medium">Адрес доставки</p>
                <p class="text-sm text-muted-foreground">{{ formatAddress(order.delivery_address) }}</p>
              </div>
            </div>

            <!-- Оплата -->
            <div class="flex items-start gap-3">
              <CreditCard class="w-5 h-5 text-muted-foreground mt-0.5" />
              <div class="flex-1">
                <p class="text-sm font-medium">Способ оплаты</p>
                <p class="text-sm text-muted-foreground">{{ paymentMethodLabel }}</p>
              </div>
            </div>
          </CardContent>
        </Card>

        <!-- Стоимость -->
        <Card>
          <CardHeader>
            <CardTitle>Стоимость</CardTitle>
          </CardHeader>
          <CardContent class="space-y-4">
            <!-- Итого -->
            <div class="flex justify-between items-center text-lg font-semibold">
              <span>Итого:</span>
              <span>{{ order.final_amount.toLocaleString('ru-RU') }} ₸</span>
            </div>

            <!-- Бонусы (только для авторизованных) -->
            <div v-if="orderTableName === 'orders' && (order.bonuses_spent > 0 || order.bonuses_awarded > 0)" class="pt-4 border-t space-y-2">
              <div v-if="order.bonuses_spent > 0" class="flex items-center justify-between text-sm">
                <div class="flex items-center gap-2 text-orange-600">
                  <Gift class="w-4 h-4" />
                  <span>Списано бонусов</span>
                </div>
                <span class="font-medium text-orange-600">-{{ order.bonuses_spent }}</span>
              </div>
              <div v-if="order.bonuses_awarded > 0" class="flex items-center justify-between text-sm">
                <div class="flex items-center gap-2 text-green-600">
                  <Gift class="w-4 h-4" />
                  <span>Начислено бонусов</span>
                </div>
                <span class="font-medium text-green-600">+{{ order.bonuses_awarded }}</span>
              </div>
            </div>
          </CardContent>
        </Card>
      </div>

      <!-- Товары в заказе -->
      <Card>
        <CardHeader>
          <CardTitle>Товары ({{ order.order_items.length }})</CardTitle>
        </CardHeader>
        <CardContent>
          <div class="space-y-4">
            <div
              v-for="item in order.order_items"
              :key="item.id"
              class="flex gap-4 p-4 border rounded-lg hover:bg-muted/50 transition-colors"
            >
              <!-- Изображение -->
              <div class="flex-shrink-0 w-20 h-20 bg-muted rounded-lg overflow-hidden">
                <ProgressiveImage
                  v-if="item.product?.product_images?.[0]"
                  :src="item.product.product_images[0].image_url"
                  :alt="item.product.name"
                  :blur-data-url="item.product.product_images[0].blur_placeholder"
                  aspect-ratio="square"
                  object-fit="cover"
                  placeholder-type="lqip"
                  eager
                />
                <div v-else class="w-full h-full flex items-center justify-center">
                  <Package class="w-8 h-8 text-muted-foreground" />
                </div>
              </div>

              <!-- Информация -->
              <div class="flex-1 min-w-0">
                <h3 class="font-medium text-sm mb-1 truncate">
                  {{ item.product?.name }}
                </h3>
                <p class="text-sm text-muted-foreground">
                  {{ item.product?.price.toLocaleString('ru-RU') }} ₸ × {{ item.quantity }}
                </p>
              </div>

              <!-- Сумма -->
              <div class="flex-shrink-0 text-right">
                <p class="font-semibold">
                  {{ (item.product?.price * item.quantity).toLocaleString('ru-RU') }} ₸
                </p>
              </div>
            </div>
          </div>
        </CardContent>
      </Card>

      <!-- Кнопки управления -->
      <div class="pt-4 flex flex-wrap gap-3">
        <Button variant="outline" @click="router.push('/')">
          <Icon name="lucide:arrow-left" class="w-4 h-4 mr-2" />
          На главную
        </Button>

        <!-- Кнопка отмены (только для new и confirmed) -->
        <Button
          v-if="canCancelOrder"
          variant="destructive"
          @click="showCancelDialog = true"
        >
          <XCircle class="w-4 h-4 mr-2" />
          Отменить заказ
        </Button>
      </div>
    </div>

    <!-- Диалог подтверждения отмены -->
    <AlertDialog v-model:open="showCancelDialog">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Отменить заказ?</AlertDialogTitle>
          <AlertDialogDescription>
            Вы уверены, что хотите отменить заказ №{{ orderId.slice(-6) }}?
            <template v-if="orderTableName === 'orders'">
              <br><br>
              Потраченные бонусы будут возвращены на ваш счёт.
            </template>
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel :disabled="isCancelling">
            Нет, оставить
          </AlertDialogCancel>
          <AlertDialogAction
            @click="handleCancelOrder"
            :disabled="isCancelling"
            class="bg-destructive text-destructive-foreground hover:bg-destructive/90"
          >
            <span v-if="isCancelling">Отменяем...</span>
            <span v-else>Да, отменить</span>
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  </div>
</template>
