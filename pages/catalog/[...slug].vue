...
const activeBrandSlug = computed(() => {
  ...
  if (!brandParam || Array.isArray(brandParam)) return null;
...
const filteredProductLines = computed(() => {
  ...
  if (selectedBrands.length === 0) return availableProductLines.value;
...
const currentCategory = computed(() => {
  if (!categoriesStore.allCategories.length) return null;
...
const categoryName = computed(() => {
  ...
  const crumbs = categoriesStore.getBreadcrumbs(currentCategorySlug.value);
  if (crumbs && crumbs.length > 0) return crumbs[crumbs.length - 1]?.name;
  return currentCategorySlug.value?.replace(/-/g, " ") || "Каталог";
});

const activeBrand = computed(() => {
  if (!activeBrandSlug.value || availableBrands.value.length === 0) return null;
  return (
...
function getArrayFromQuery(
  ...
  if (Array.isArray(queryValue)) return queryValue.filter(Boolean) as string[];
...
function getSortByFromQuery(
  ...
  if (!queryValue) return "popularity";
...
watch(
  () => activeFilters.value.brandIds,
  (newBrandIds) => {
    if (newBrandIds.length === 0) return;
...
function updateAttribute(
  ...
  if (checked) newSelection.add(stringId);
...
const selectedSingleLine = computed(() => {
  if (activeFilters.value.productLineIds.length !== 1) return null;
  return (
...
const selectedSingleBrand = computed(() => {
  if (activeFilters.value.brandIds.length !== 1) return null;
  return (
...
const metaDescription = computed(() => {
  ...
  parts.push(
    "купить в интернет-магазине Ухтышка ✔️ Большой выбор, доставка по Алматы и Казахстану",
  );
  return parts.join(". ");
});
...
const { data: categoryQuestions } = await useAsyncData(
  ...
  async () => {
    ...
    if (!category?.id || currentCategorySlug.value === "all") return [];
      ...
      <div class="col-span-1 lg:col-span-3 min-w-0">
        ...
        >
          <div :key="isLoading ? 'loading' : 'content'">
            ...
            >
              <h3 class="text-2xl font-semibold">Товары не найдены</h3>
              <p class="mt-2">
