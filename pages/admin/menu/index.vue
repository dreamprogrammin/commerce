...
const rootCategories = computed(() =>
  adminCategoriesStore.buildCategoryTree(null),
)
...
async function saveAllChanges() {
  ...
  if (success && selectedRootCategory.value) {
    const updatedRoot = adminCategoriesStore.allCategories.find(
      c => c.id === selectedRootCategory.value?.id,
    )
    if (updatedRoot) {
      const fullTree = adminCategoriesStore.buildCategoryTree(null)
      selectedRootCategory.value
        = fullTree.find(c => c.id === updatedRoot.id) || null
    }
  }
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-4 gap-8">
      ...
      <aside class="lg:col-span-1">
          ...
          <Button
            ...
            :class="{
              'bg-accent text-accent-foreground font-semibold':
                selectedRootCategory?.id === rootCat.id,
            }"
        ...
        <p class="text-xs text-muted-foreground mt-2">
          Корневые категории создаются в базе данных с флагом `is_root_category
          = true`.
        </p>
      </aside>
      ...
      <main class="lg:col-span-3">
        ...
        >
          ...
          <div
            ...
            class="text-center py-5 border-dashed border-2 rounded-md"
          >
            Нет подкатегорий. Нажмите кнопку ниже, чтобы добавить.
          </div>

          <div
            v-for="(item, index) in formTree"
